...
fn main() {
    println!("{:?}", chunkify(String::from("abc")));
    println!("{}", chunkify(String::from("abc")).len() * 32 * 16);
    println!("{:#b}, {:#b}", rot_r(0x9fffffffu32, 26), rot_l(0x9fffffffu32, 6));
}

fn sha256(message: String) -> [u32; 8] {
    let mut h = H0_SHA256.clone();
    for chunk in chunkify(message) {
